...
- `deno task update` / `update:write` は `--no-lock` で起動する。molt は Deno 2 の lockfile (v5) を読めず、`--no-lock` を付けないと molt 自身の依存 (`@molt/cli` 等) がプロジェクトの `deno.lock` に混入してしまうため。lockfile は molt ではなく手順 3 (`deno task check`) / 手順 5 (`deno install`) で、いったん削除してから再生成する。
